...
public class SNTUI extends JDialog {
    ...
    private JMenu fileMenu(final ScriptInstaller installer) {
        ...
        ScriptRecorder.setRecordingCall(fromList, "snt.getUI().runCommand(\"From Open Image...\")");
        ...
        saveAndOpenPrev.setToolTipText("Saves current tracings alongside the image file,\nthen loads the previous image (alphabetically) in the same folder");
        ...
        ScriptRecorder.setRecordingCall(importFlyCircuit, "snt.getUI().runCommand(\"FlyCircuit...\")");
        ...
        ScriptRecorder.setRecordingCall(importInsectBrainDb, "snt.getUI().runCommand(\"InsectBrain...\")");
        ...
        ScriptRecorder.setRecordingCall(importMouselight, "snt.getUI().runCommand(\"MouseLight...\")");
    ...
    private JMenu analysisMenu() {
        ...
        ScriptRecorder.setRecordingCall(shollMenuItem, "snt.getUI().runCommand(\"Sholl Analysis (Tracings)...\")");
    ...
    private JMenu utilitiesMenu() {
        ...
        ScriptRecorder.setRecordingCall(graphGenerator, "snt.getUI().runCommand(\"Create Dendrogram\")");
    ...
    private JMenu getBrainAnnotationMenu() {
        ...
        ScriptRecorder.setRecordingCall(mi, "snt.getUI().runCommand(\"Create Annotation Graph\")");
        menu.add(mi);
        ...
        ScriptRecorder.setRecordingCall(mi, "snt.getUI().runCommand(\"Show Brain Area Info...\")");
        menu.add(mi);
        return menu;
    }
    ...
    private JMenuItem shollAnalysisHelpMenuItem() {
        ...
        mi.addActionListener(e -> {
            final Thread newThread = new Thread(() -> {
                ...
                sb.append("<p><b>Important:</b> If one or more paths are currently selected in the Path ");
                sb.append("Manager, the search for the nearest node is restricted to those paths only, ");
                sb.append("even if a closer node exists elsewhere. If the node you clicked near does not ");
                sb.append("seem to be picked up, clear the selection in the Path Manager first (use its ");
                sb.append("right-click <i>Deselect / Select All</i> command) so that all paths become eligible, ");
                sb.append("or make sure the path holding the node of interest is part of the selection.</p>");
                sb.append("<p>The node that will actually be used as the center of analysis will be highlighted ");
                sb.append("on the canvas once resolved.</p>");
    ...
    /**
     * Registers a menu item for script recording and attaches a DynamicCmdRunner action.
     * This ensures the menu action is logged by the ScriptRecorder when executed.
     *
     * @param menuItem       the menu item to register
     * @param cmd            the Command class to run
     * @param inputsSupplier supplier for command inputs (can be null)
     * @param uiState        the UI state during command execution
     */
    private void addDynamicCmdAction(final AbstractButton menuItem, final Class<? extends Command> cmd,
                                     final Supplier<HashMap<String, Object>> inputsSupplier, final int uiState) {
        final String label = (menuItem.getActionCommand() != null) ? menuItem.getActionCommand() : menuItem.getText();
        ScriptRecorder.setRecordingCall(menuItem, "snt.getUI().runCommand(\"" + label + "\")");
        menuItem.addActionListener(e -> {
            final HashMap<String, Object> inputs = (inputsSupplier != null) ? inputsSupplier.get() : null;
            new DynamicCmdRunner(cmd, inputs, uiState).run();
        });
    }
    ...
    protected void saveTracingsAndOpenSiblingImage(final boolean next) {
        ...
        if (imp == null || imp.getOriginalFileInfo() == null) {
            error("Current image has no associated file. Please save the image first.");
            return;
        }
        final File currentFile = new File(imp.getOriginalFileInfo().directory, imp.getOriginalFileInfo().fileName);
        if (!currentFile.exists()) {
            error("Could not determine location of current image file.");
            return;
        }
        final List<String> extensions = plugin.getPrefs().getNextImgExtensions();
        final File dir = currentFile.getParentFile();
        // Collect and sort image files in the same directory
        final File[] siblings = dir.listFiles(f -> {
            if (!f.isFile()) return false;
            final String name = f.getName().toLowerCase();
            return extensions.stream().anyMatch(name::endsWith);
        });
        if (siblings == null || siblings.length == 0) {
            error("No image files found in " + dir.getAbsolutePath());
            return;
        }
        // Explicit alphabetical sort: in APFS filesystems etc., order is not reliable (hash sorting)
        Arrays.sort(siblings, Comparator.comparing(f -> f.getName().toLowerCase()));

        // Find current index
        int idx = -1;
        for (int i = 0; i < siblings.length; i++) {
            if (siblings[i].getName().equals(currentFile.getName())) {
                idx = i;
                break;
            }
        }
        if (idx == -1) {
            error("Could not locate current image in folder listing.");
            return;
        }
        final int targetIdx = next ? idx + 1 : idx - 1;
        if (targetIdx < 0 || targetIdx >= siblings.length) {
            error((next ? "No next" : "No previous") + " image in folder. Current image is the " +
                    (next ? "last." : "first."));
            return;
        }

        // Save tracings alongside current image (same directory, same name stem, .traces extension)
        final File targetFile = siblings[targetIdx];
        if (pathAndFillManager.size() > 0) {
            final File tracesFile = SNTUtils.getUniquelySuffixedFile(currentFile, ".traces");
            if (!saveToXML(tracesFile, false)) {
                if (!guiUtils.getConfirmation(
                        "Tracings could not be saved. Proceed to next image anyway?",
                        "Save Failed")) {
                    return;
                }
            }
        ...
        plugin.getPrefs().setAutosaveFile(null);
