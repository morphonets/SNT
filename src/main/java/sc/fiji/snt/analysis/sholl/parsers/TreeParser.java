...
public class TreeParser implements Parser {
	 ...
	 * </p>
	 * <p><b>Important:</b> This method must be called <em>before</em> {@link #parse()}.
	 * @param b whether to store the intersected volume as an extra measurement
	 */
	...
	private double calculateSegmentLengthInShell(final PointInImage node1, final PointInImage node2,
		...
		// Collect t-boundary values from both sphere intersections, plus segment endpoints
		final List<Double> tList = new ArrayList<>(6);
		tList.add(0.0);
		tList.add(1.0);
		for (final double r : new double[]{innerRadius, outerRadius}) {
			if (r <= 0) continue;
			for (final double t : findLineSphereIntersection(node1, node2, center, r)) {
				if (t > 0.0 && t < 1.0) tList.add(t);
			}
		}
		Collections.sort(tList);

		// Sum sub-intervals whose midpoint lies within the shell
		double total = 0.0;
		final double inner2 = innerRadius * innerRadius;
		final double outer2 = outerRadius * outerRadius;
		for (int k = 0; k < tList.size() - 1; k++) {
			final double t0 = tList.get(k);
			final double t1 = tList.get(k + 1);
			if (t1 <= t0) continue;
			final double tm = 0.5 * (t0 + t1);
			final double mx = node1.x + tm * (node2.x - node1.x) - center.x;
			final double my = node1.y + tm * (node2.y - node1.y) - center.y;
			final double mz = node1.z + tm * (node2.z - node1.z) - center.z;
			final double r2m = mx*mx + my*my + mz*mz;
			if (r2m >= inner2 && r2m <= outer2) {
				total += t1 - t0;
			}
		}
		return segmentLength * total;
	}
	...
	/**
	 * Gets intersection points at a specific radius.
	 *
