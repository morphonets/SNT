 ...
 */
@Plugin(type = Command.class, label="Root Angle Analysis...")
public class RootAngleAnalyzerCmd extends ContextCommand {
	...
	// Define the GUI. This could be a proper interactive like TreeColorMapperCmd, but for now it will do
	@Parameter(required=false, label = "Heatmap LUT", choices = { "None", "Fire", "Ice", "Plasma", "Viridis", "Red", "Blue" })
	...
	public void run() {
		...
		if (plots) showGraphs();
		if (!"None".equals(lutChoice)) runFigCreator();
	}
	...
	private void showTabularSummary() {
		final Display<?> tableDisplay = displayService.getDisplay("SNT Root Angle Table");
	...
	private void runFigCreator() {
		...
		for (final RootAngleParser parser : parsers) {
		 ...
		 mapper.map(TreeColorMapper.VALUES, getColorTable());
	...
	private ColorTable getColorTable() {
        return switch (lutChoice) {
			case "Blue" -> ColorTables.BLUE;
			case "Red" -> ColorTables.RED;
            case "Fire" -> ColorTables.FIRE;
            case "Viridis" -> ColorMaps.VIRIDIS;
            case "Plasma" -> ColorMaps.PLASMA;
            default -> ColorMaps.ICE;
        };
	}
